...
const GraphEditorPanel: React.FC<GraphEditorPanelProps> = (props: GraphEditorPanelProps) => {
    ...
    return (
        <div id='container' ref={ref}>
            <svg id='canvas'></svg>
            <button id='fit' className='overlay-button hidden' title='Fit view' onClick={() => { editorRef.current?.fit(); }}>
                <FontAwesomeIcon icon={faUpRightAndDownLeftFromCenter} size='sm' />
            </button>
			<button id='show-handle' className='overlay-button hidden' title='Show Handle' onClick={() => { config.showHandle = true; editorRef.current?.updateHandleVisibility(); }}>
